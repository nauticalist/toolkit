...
func TestTools_UploadFiles(t *testing.T) {
	for _, e := range uploadTests{
		...
		if !e.errorExpected && err != nil {
			t.Errorf("%s: error expected bu not received", e.name)
		}
...
func TestTools_DownloadStaticFile(t *testing.T) {
	...
	testTool.DownloadStaticFiles(rr, req, "./testdata", "file.jpg", "fire.jpg")

	res := rr.Result()
	defer res.Body.Close()

	if res.Header["Content-Length"][0] != "267158" {
		t.Error("wrong content length of", res.Header["Content-Length"][0])
	}

	if res.Header["Content-Disposition"][0] != "attachment; filename=\"fire.jpg\"" {
